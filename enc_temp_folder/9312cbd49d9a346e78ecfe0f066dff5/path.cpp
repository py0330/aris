...
namespace aris::plan{
		...
		double& arc, double& darc, double &d2arc)noexcept->void
	{
		...
		d2arc = a * s * s * 3 + b * s * 2 + c - e * std::sin(2 * PI * s) * 2 * PI;
	}

	
	auto s_bezier3_arc2s(double arc, double darc, double d2arc, double a, double b, double c, double d, double e, double& s, double& ds, double& d2s)noexcept->void {
		double A = a / 4.0;
		double B = b / 3.0;
		double C = c / 2.0;
		double E = e / 2.0/PI;
		...
		double darc_ds = a * s * s * s + b * s * s + c * s + d + e * std::cos(two_pi * s);
		double d2arc_ds2 = a * s * s * 3 + b * s * 2 + c - e * std::sin(two_pi * s) * two_pi;
