...
stateDiagram-v2
    ...
    PausedGoto --> Stopping : requestStop()
    UninitializedGoto --> Stopping : requestStop()
    Stopping --> Stopping : getNextInput（内部 stopOneStep 减速中）
    Stopping --> Uninitialized : getNextInput（内部 stopOneStep 速度降到 0）

    %% goto 目标（独立管线：专用 tg/is/sr，单指令）
    Uninitialized --> UninitializedGoto : gotoJ / gotoAbsJ / gotoL / gotoC
    Paused --> PausedGoto : gotoJ / gotoAbsJ / gotoL / gotoC
    UninitializedGoto --> UninitializedGoto : getNextInput（goto 专用管线推进）
    PausedGoto --> PausedGoto : getNextInput（goto 专用管线推进）
    UninitializedGoto --> Uninitialized : getNextInput（到达目标）
    PausedGoto --> Paused : getNextInput（到达目标）
...
> goto 目标（`gotoJ` / `gotoAbsJ` / `gotoL` / `gotoC`）使用独立管线（专用 tg/is/sr，单指令，zone=0），
> 不插入主队列。仅允许从 `Uninitialized`（→`UninitializedGoto`）或 `Paused`（→`PausedGoto`）启动；
> 已在 goto 状态时不能重复插入。

## 说明

- **稳定态**：`Idle`、`Paused`、`Uninitialized` 是三个“停住”的状态；`Stopping` / `Pausing` / `Resuming` / `PausedGoto` / `UninitializedGoto` 是过程态，每个 RT 周期推进一步，完成后跳到各自终态。
- **`PausedGoto` / `UninitializedGoto`**：goto 的专用管线状态，完成后分别回到 `Paused` / `Uninitialized`。
