...
namespace aris
{
	...
	namespace dynamic
	{
		auto inline acc_up(int n, int i)noexcept->double{return (-1.0 / 2 / n / n / n * i*i*i + 3.0 / 2 / n / n * i*i);}
		auto inline acc_down(int n, int i)noexcept->double{	return (-1.0*i*i*i / 2.0 / n / n / n + 3.0 * i*i / 2 / n / n);}
		auto inline dec_up(int n, int i)noexcept->double{return 1 - (-1.0 / 2 / n / n / n * (n - i)*(n - i)*(n - i) + 3.0 / 2 / n / n * (n - i)*(n - i));}
		auto inline dec_down(int n, int i)noexcept->double
		{
			return 1 - (-1.0*(n - i)*(n - i)*(n - i) / 2.0 / n / n / n + 3.0 * (n - i)*(n - i) / 2 / n / n);
		}

		auto inline acc_even(int n, int i)noexcept->double
		{
			return 1.0 / n / n  * i * i;
		}
		auto inline dec_even(int n, int i)noexcept->double
		{
			return 1.0 - 1.0 / n / n * (n - i)*(n - i);
		}
		auto inline even(int n, int i)noexcept->double
		{
			return 1.0 / n*i;
		}
