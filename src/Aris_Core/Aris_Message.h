...
namespace Aris
{
	namespace Core
	{
		...
		*/
		//int RegisterMsgCallback(unsigned int message, int (*CallBack)(Aris::Core::MSG &));
		int RegisterMsgCallback(unsigned int message, std::function<int(Aris::Core::MSG &)>);
		/** \brief 注册默认回调函数
		* 当系统碰到没有注册过的消息标识时，就执行默认的回调函数。
		* \param CallBack 默认回调函数，该函数应当形如 int CallBack(Aris::Message::MSG & msg){};
		*/
		int RegisterDefaultCallback(std::function<int(Aris::Core::MSG &)>);
		/** \brief 主线程开始消息循环
		*
		*/
		int RunMsgLoop();
