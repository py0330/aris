...
namespace aris
{
	namespace plan
	{
		...
		{
			aris::core::ObjectPool<ParamBase>::saveXml(xml_ele);
		...
		{
			aris::core::ObjectPool<ParamBase>::loadXml(xml_ele);
		...
		struct Command::Imp
		{
			...
			static auto take(aris::core::Object* param)->void
			{
			...
			static auto reset(aris::core::Object* param)->void
			{
				if (dynamic_cast<Param*>(param))
				{
			...
			static auto addDefaultParam(aris::core::Object* param, std::map<std::string, std::string> &param_map_out)->void
			{
				if (dynamic_cast<Param*>(param))
				{
					auto p = dynamic_cast<Param*>(param);
		...
		{
			registerType<aris::plan::Param>();
			registerType<aris::plan::UniqueParam>();
			registerType<aris::plan::GroupParam>();
		}
		...
		struct CommandParser::Imp { aris::core::ObjectPool<Command>* command_pool_; };
		...
		auto CommandParser::commandPool()->aris::core::ObjectPool<Command> & { return *imp_->command_pool_; }
