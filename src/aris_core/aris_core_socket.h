	...
	namespace core
	{
		...
		public:
			...
			public:
				...
				int id_;
			private:
				StartServerError(const char* what, Socket2 *socket_in, int id) : runtime_error(what), socket_(socket_in), id_(id) {};
			...
			public:
				...
				int id_;
			private:
				ConnectError(const char* what, Socket2 *socket, int id) : runtime_error(what), socket_(socket), id_(id) {}
				friend class Socket2;
			};
			class SendDataError :public std::runtime_error
			{
			public:
				...
				int id_;
			private:
				SendDataError(const char* what, Socket2 *socket, int id) : runtime_error(what), socket_(socket), id_(id) {}
				friend class Socket2;
			};
			class SendRequestError :public std::runtime_error
			{
			public:
				...
				int id_;
			private:
				SendRequestError(const char* what, Socket2 *socket, int id) : runtime_error(what), socket_(socket), id_(id) {}
				friend class Socket2;
			};

		public:
			static auto Type()->const std::string &{ static const std::string type("Socket2"); return std::ref(type); }
			auto virtual type() const->const std::string& override{ return Type(); }
			auto virtual loadXml(const aris::core::XmlElement &xml_ele)->void override;
			auto isConnected()->bool;
			auto state()->State;
			...
			auto connect(const std::string &remote_ip = std::string(), const std::string &port = std::string())->void;
			auto stop()->void;
			auto sendMsg(const std::string &data)->void;
			auto remoteIP()const->const std::string &;
			auto port()const->const std::string &;
			auto setRemoteIP(const std::string &remote_ip)->void;
			auto setPort(const std::string &port)->void;
			auto setOnReceivedMsg(std::function<int(Socket2*, aris::core::Msg &)> = nullptr)->void;
			auto setOnReceivedConnection(std::function<int(Socket2*, const char* remote_ip, int remote_port)> = nullptr)->void;
			auto setOnLoseConnection(std::function<int(Socket2*)> = nullptr)->void;
			auto setOnReceivedRequest(std::function<aris::core::Msg(Socket2*, aris::core::Msg &)> = nullptr)->void;
			auto setOnAcceptError(std::function<void(Socket2*)> = nullptr)->void;
			auto setOnReceiveError(std::function<void(Socket2*)> = nullptr)->void;

			virtual ~Socket2();
			Socket2(const std::string &name = "socket", const std::string& remote_ip = "", const std::string& port = "");
			Socket2(const Socket2 & other) = delete;
			Socket2(Socket2 && other) = default;
			Socket2 &operator=(const Socket2& other) = delete;
			Socket2 &operator=(Socket2&& other) = default;
