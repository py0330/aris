...
namespace aris
{
	namespace core
	{
		...
		{
			Socket2* socket_;
			Socket2::State state_;

			std::function<aris::core::Msg(Socket2 *, aris::core::Msg &)> onReceivedRequest;
			std::function<int(Socket2 *, aris::core::Msg &)> onReceivedData;
			std::function<int(Socket2 *, const char *, int)> onReceivedConnection;
			std::function<int(Socket2 *)> onLoseConnection;
...
#endif
			~Imp() = default;
			Imp(Socket2* sock) :socket_(sock), lisn_socket_(0), conn_socket_(0), sin_size_(sizeof(struct sockaddr_in)), state_(Socket2::IDLE)
			...
			static void receiveThread(Socket2::Imp* imp, std::promise<void> receive_thread_ready);
			static void acceptThread(Socket2::Imp* imp, std::promise<void> accept_thread_ready);
		};
		auto Socket2::Imp::acceptThread(Socket2::Imp* imp, std::promise<void> accept_thread_ready)->void
		{
			...
			// 以下从对象中copy内容,此时start_Server在阻塞,因此Socket2内部数据安全 //
		...
		auto Socket2::Imp::receiveThread(Socket2::Imp* imp, std::promise<void> receive_thread_ready)->void
		{
			...
			for (;;)
			{
				int res = recv(conn_socket, msg.data(), sizeof(MsgHeader), 0);
				...
				if (imp->onReceivedData)imp->onReceivedData(imp->socket_, msg);
			}
		}
		auto Socket2::loadXml(const aris::core::XmlElement &xml_ele)->void
		{
		...
		auto Socket2::stop()->void
		{
			...
			imp_->state_ = Socket2::IDLE;
		}
		auto Socket2::isConnected()->bool
		{
		...
		auto Socket2::state()->State 
		{ 
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			return imp_->state_;
		};
		auto Socket2::startServer(const std::string &port)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);

			if (!port.empty())setPort(port);
			if(this->port().empty())throw StartServerError("Socket2 can't Start as server, because it has empty port\n", this, 0);
			...
			case IDLE:
				break;
			default:
				throw(StartServerError("Socket2 can't Start as server, because it is not at idle state\n", this, 0));
			}

			// 启动服务器 //
#ifdef WIN32 
			if (WSAStartup(0x0101, &imp_->wsa_data_) != 0)throw(StartServerError("Socket2 can't Start as server, because it can't WSAstartup\n", this, 0));
#endif

			// 服务器端开始建立socket描述符 //
			if ((imp_->lisn_socket_ = socket(AF_INET, SOCK_STREAM, 0)) == -1)throw(StartServerError("Socket2 can't Start as server, because it can't socket\n", this, 0));
			...
			if (::setsockopt(imp_->lisn_socket_, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char*>(&nvalue), sizeof(int)) < 0)
				throw StartServerError("Socket2 can't set REUSEADDR option\n", this, 0);
...
#endif
				throw(StartServerError("Socket2 can't Start as server, because it can't bind\n", this, 0));
			}

			// 监听lisn_socket_描述符 //
			if (listen(imp_->lisn_socket_, 5) == -1)throw(StartServerError("Socket2 can't Start as server, because it can't listen\n", this, 0));

			// 启动等待连接的线程 //
			std::promise<void> accept_thread_ready;
			auto ready = accept_thread_ready.get_future();
			imp_->recv_conn_thread_ = std::thread(Socket2::Imp::acceptThread, this->imp_.get(), std::move(accept_thread_ready));
			ready.wait();

			return;
		}
		auto Socket2::connect(const std::string &remote_ip, const std::string &port)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);

			if (!remote_ip.empty())setRemoteIP(remote_ip);
			if (!port.empty())setPort(port);
			if (remoteIP().empty())throw ConnectError("Socket2 can't connect, because it empty ip address\n", this, 0);
			if (this->port().empty())throw ConnectError("Socket2 can't connect, because it empty port\n", this, 0);
			...
			case IDLE:
				break;
			default:
				throw ConnectError("Socket2 can't connect, because it is busy now, please close it\n", this, 0);
			}

			// 启动服务器 //
#ifdef WIN32
			if (WSAStartup(0x0101, &imp_->wsa_data_) != 0)throw ConnectError("Socket2 can't connect, because can't WSAstartup\n", this, 0);
#endif
			// 服务器端开始建立socket描述符 //
			if ((imp_->conn_socket_ = socket(AF_INET, SOCK_STREAM, 0)) < 0)throw ConnectError("Socket2 can't connect, because can't socket\n", this, 0);
			...
			if (::connect(imp_->conn_socket_, (const struct sockaddr *)&imp_->server_addr_, sizeof(imp_->server_addr_)) == -1)
				throw ConnectError("Socket2 can't connect, because can't connect\n", this, 0);

			imp_->state_ = Socket2::WORKING;
		...
		auto Socket2::sendMsg(const std::string &data)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);

			if (send(imp_->conn_socket_, data.c_str(), data.size(), 0) == -1)
				throw SendDataError("Socket2 failed sending data, because network failed\n", this, 0);
		}
		auto Socket2::remoteIP()const->const std::string &
		{ 
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			return imp_->remote_ip_;
		}
		auto Socket2::port()const->const std::string &
		{ 
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			return imp_->port_; 
		}
		auto Socket2::setRemoteIP(const std::string &remote_ip)->void 
		{ 
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->remote_ip_ = remote_ip; 
		}
		auto Socket2::setPort(const std::string &port)->void 
		{ 
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->port_ = port; 
		}
		auto Socket2::setOnReceivedMsg(std::function<int(Socket2*, aris::core::Msg &)> OnReceivedData)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onReceivedData = OnReceivedData;
		}
		auto Socket2::setOnReceivedRequest(std::function<aris::core::Msg(Socket2*, aris::core::Msg &)> OnReceivedRequest)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onReceivedRequest = OnReceivedRequest;
		}
		auto Socket2::setOnReceivedConnection(std::function<int(Socket2*, const char*, int)> OnReceivedConnection)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onReceivedConnection = OnReceivedConnection;
		}
		auto Socket2::setOnLoseConnection(std::function<int(Socket2*)> OnLoseConnection)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onLoseConnection = OnLoseConnection;
		}
		auto Socket2::setOnAcceptError(std::function<void(Socket2*)> onAcceptError)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onAcceptError = onAcceptError;
		}
		auto Socket2::setOnReceiveError(std::function<void(Socket2*)> onReceiveError)->void
		{
			std::unique_lock<std::recursive_mutex> lck(imp_->state_mutex_);
			imp_->onReceiveError = onReceiveError;
		}
		Socket2::~Socket2() {	stop();	}
		Socket2::Socket2(const std::string &name, const std::string& remote_ip, const std::string& port):Object(name), imp_(new Imp(this))
		{
