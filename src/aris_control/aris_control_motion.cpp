...
namespace aris
{
	namespace control
	{
		...
		Motion::Motion(Object &father, std::size_t id, const aris::core::XmlElement &xml_ele):SlaveTemplate(father, id, xml_ele)
		{
		...
		auto Motion::readData(RxType &rx_data)->void
		{
			std::int32_t value;
			readPdoIndex(0x6064, 0x0000, value);
			rx_data.pos = static_cast<double>(value) / imp_->input2count_;
		}
		auto Motion::writeData(const TxType& tx_data)->void
		{
