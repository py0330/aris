...
namespace aris::core{
	...
	auto SocketServer::startServer(const std::string& port) -> int {
		...
		if (sock_type == SOCK_STREAM) {
			...
			if (setsockopt(imp_->lisn_socket_, SOL_SOCKET, SO_RCVTIMEO, (void*)&__tcp_tv, sizeof(__tcp_tv)) < 0) {
				aris_close(imp_->lisn_socket_);
				ARIS_LOG(SOCKET_SERVER_START_ERROR, "setsockopt SO_RCVTIMEO FAILED (fallback for TCP_USER_TIMEOUT)");
				return -5;
			}
			if (setsockopt(imp_->lisn_socket_, SOL_SOCKET, SO_SNDTIMEO, (void*)&__tcp_tv, sizeof(__tcp_tv)) < 0) {
				aris_close(imp_->lisn_socket_);
				ARIS_LOG(SOCKET_SERVER_START_ERROR, "setsockopt SO_SNDTIMEO FAILED (fallback for TCP_USER_TIMEOUT)");
				return -5;
			}
#endif
