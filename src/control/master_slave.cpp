	...
	public:
		...
		static auto rt_task_func(void* master)->void {
			...
			while (mst.imp_->is_rt_thread_running_) {
				...
				std::int64_t ns_wakeup, ns_series[10], ns_total, series_id{ 0 };
				char ns_name[10][10];
				...
				if (link_up) {
					...
					mst.recv();
#ifdef DEBUG_MASTER_TIME
					ns_series[series_id + 1] = aris::control::aris_rt_time_since_last_time();
					std::strcpy(ns_name[series_id + 1], "recv   : ");
					++series_id;
#endif
					// queue //
					mst.queue();
#ifdef DEBUG_MASTER_TIME
					ns_series[series_id + 1] = aris::control::aris_rt_time_since_last_time();
					std::strcpy(ns_name[series_id + 1], "queue  : ");
					++series_id;
#endif
...
#ifdef DEBUG_MASTER_TIME
					ns_series[series_id + 1] = aris::control::aris_rt_time_since_last_time();
					std::strcpy(ns_name[series_id + 1], "send   : ");
					++series_id;
#endif
...
#ifdef DEBUG_MASTER_TIME
				if (ns_wakeup > 100000 || ns_total > 800000) {
					mst.mout() << "********************************************\n";
					for (int i = 0; i < series_id; ++i)
						mst.mout() << ns_name[i] << ns_series[i] << "\n";
					mst.mout() << "total  : " << ns_total << "\n";
					mst.mout() << "********************************************\n" << std::endl;
				}
#endif
			}
