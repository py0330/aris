...
namespace aris::plan {
    ...
    auto s_acc_vend(double va, double a, double j, double T)noexcept->double {
        return a / j > T / 2 ? va + j * T * T / 4 : va + T * a - a * a / j;
    };
    // 给定速度，计算加加速段的终止速度
	...
	// 计算指定时间处的 p v a j
	auto s_s_curve_at(const SCurveParam& param, double t, double* p_out, double* v_out, double* a_out, double* j_out)noexcept->void {
	...
	// 主要计算vb，v，Ta，Tb等信息
	// 注意，真实的v_max <= 输入的v_max
	auto s_compute_s_curve(SCurveParam& param)->void {
        const double va     = std::max(param.va_, 0.0);
        const double vb_max = param.vb_max_;
        const double vc_max = param.vc_max_;
        const double a      = param.a_;
        ...
        const double t_cons = 100000.0 * std::max(T, 1.0) * std::numeric_limits<double>::epsilon();
...
//% 条件 B.4 可得 Ta >= 0
	...
	// 主要计算vb，v，Ta，Tb等信息
	// 注意，真实的v_max <= 输入的v_max
    auto s_compute_s_curve_Tmax_Tmin(const SCurveParam& param)->std::tuple<double, double> {
        ...
        if (pt < l) {
            ...
            double v = newton_raphson_binary_search([v1,v2,j,pt](double x) {
                return std::sqrt((x - v1) / j) * (v1 + x) + std::sqrt((x - v2) / j) * (v2 + x) - pt;
                }
                
                , v_below, v_upper);

            double T1 = 2 * std::sqrt((v - v1) / j);
            double T2 = 2 * std::sqrt((v - v2) / j);
            Tmin = T1 + T2;
            return std::make_tuple(Tmax, Tmin);
        }
        ...
        if (pt < l){
            ...
            //% 
            double v = newton_raphson_binary_search([v1, v2, a, j, pt](double x)->double {
                return std::sqrt((x - v1) / j) * (v1 + x) + ((x - v2) / a + a / j) * (v2 + x) / 2 - pt;
                }
                , v_below, v_upper);
    ...
    auto test_curve_slow(std::list<SCurveNode>::iterator begin, std::list<SCurveNode>::iterator end)->bool {
        ...
        for (Size i = 0; i < begin->params_.size(); ++i) {
            // 计算 vb
            s_compute_s_curve(begin->params_[i]);

            // 更新 vb
            std::next(begin)->params_[i].va_ = begin->params_[i].vb_;

            // 计算 Tmax Tmin
            std::tie(Tmaxs[i], Tmins[i]) = s_compute_s_curve_Tmax_Tmin(std::next(begin)->params_[i]);
        }
    ...
    // 循环计算每个节点：
    auto SCurveStruct::compute()->void {
        auto begin_iter = nodes_.begin();
        auto end_iter = nodes_.end();
        ...
        for (auto iter = begin_iter; iter != end_iter; ++iter) {
            ...
            for (Size i = 0; i < iter->params_.size(); ++i) {
                ...
                // 计算 Tmax Tmin
                std::tie(Tmaxs[i], Tmins[i]) = s_compute_s_curve_Tmax_Tmin(iter->params_[i]);
            }
